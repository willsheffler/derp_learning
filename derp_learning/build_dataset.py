...
def process_pdb_data(fnames, parallel):
    ...
    pdbenergies = {
        k: np.array([v["energies"][k] for v in raw], "f4")
        for k in raw[0]["energies"]
    }
    ...
    resdata = dict()
    resdata["pdbno"] = [
        np.repeat(i, len(x["coords"]["cb"])) for i, x in enumerate(raw)
    ]
    ...
    tmp = [
        np.repeat(i, len(x["pairdata"]["p_dist"])) for i, x in enumerate(raw)
    ]
    ...
    print(
        f"returning: nstruct {len(pdb):,} nres {tot_nres:,} npairs {tot_pairs:,}"
    )
    return dict(
        pdbdata=pdbdata,
        coords=coords,
...
def compute_neighbor_stats(pdb, resdata, cb, nbdists, pdb_res_offsets,
                           parallel, **kw):
...
def sanity_check_pair_res_relation(pdb, nres, pdb_pair_offsets, pairdata,
                                   **kw):
...
def compute_pair_xform_bins(dat, parallel=-1):
    ...
    #    # xbin.XformBinner(c, o) for c, o in bin_parms
    # ]
    ...
    # if this actually needed, should be replaced with rpxdock
    #    t = perf_counter()
    #    dat["xbin_types"] = list()
    #    with get_process_executor(parallel) as pool:
