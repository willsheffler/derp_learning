...
def pdbdata(pose, fname):
   ...
   energies = {
      "t_" + k: pose.energies().total_energies()[st] for k, st in all_score_types.items()
   }
   energiesw = {
   ...
   resdata = dict(
      phi=[pose.phi(i) for i in range(1,
                                      len(pose) + 1)],
      psi=[pose.psi(i) for i in range(1,
                                      len(pose) + 1)],
      omega=[pose.omega(i) for i in range(1,
                                          len(pose) + 1)],
      chi1=[get_pose_chi(pose, i, 1) for i in range(1,
                                                    len(pose) + 1)],
      chi2=[get_pose_chi(pose, i, 2) for i in range(1,
                                                    len(pose) + 1)],
      chi3=[get_pose_chi(pose, i, 3) for i in range(1,
                                                    len(pose) + 1)],
      chi4=[get_pose_chi(pose, i, 4) for i in range(1,
                                                    len(pose) + 1)],
   )
...
def extract_pair_terms(pose, alapose, sym_chain_follows, chains, fname, min_ssep, **kw):
   ...
   lbls = ["p_dist", "p_raw_etot", "p_etot", "p_resi", "p_resj"]
   ...
   for ichain, chain in enumerate(chains):
      ...
      for ir in range(*chain):
         ...
         for jr in range(ir + 1, len(pose)):
            ...
            if etot >= 0:
               continue
            pairterms["p_raw_etot"].append(etot)
